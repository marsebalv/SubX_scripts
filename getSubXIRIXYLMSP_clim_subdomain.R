...
type='hindcast'                                                           # hindcast or forecast or climatology
...
for(ivar in 1:nvars){
  ...
  for(imodel in 1:nmodels){
    ...
    # Open File
    inFname=paste0(url,'.',group,'/.',model,'/.',type,'/.dc9915/.',varname,'/dods') # input filename
    ...
    outDir=paste0(outPath,type,'/',varname,plevstr,'/daily/clim/',group,'-',model,'/');
    ...
    for(i in 0:(nics-1)){
      ...
      ofname=paste0(outDir,varname,'_',as.character(plevstr),'_',group,'-',model,'_',substr(startdates[i+1],1,4),substr(startdates[i+1],6,7),substr(startdates[i+1],9,10),'.SISdom.daily.clim.nc')
      ...
      if(ndims == 6){ # multilevel data
        data = metR::ReadNetCDF(inFname, subset = list(P = plev, S = as.Date(startdates[i+1]), X = subdomX, Y=subdomY),out='array')
      } else{ # single level data
        data = metR::ReadNetCDF(inFname, subset = list(S = as.Date(startdates[i+1]),X = subdomX, Y=subdomY),out='array')
      }
