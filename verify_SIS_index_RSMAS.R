# SubX data - SIS index verification for RSMAS model (SubX database)
#
# M. Alvarez - 2020
#-----------------------------------------------------------------------------------------------------------------------
rm(list=ls())
...
#  Main Program  
#---------------------------------------------------------------------------------------
dt.verif = as.data.table(readRDS("/home/alvarez/SubX_processed_Rdata/toverif_RSMAS_ONDEFMA.rds"))
...
# Acá debería encontrar una forma de diferenciar las dos corridas para la misma startdate (en caso que haya)
dt.verif=dt.verif[(L.MME>0 & L.MME<50),!c("L")]

# En la variable "run" voy a guardar "1" o "2" según si hay más de una corrida por semana
dt.verif$run=1

auxmmesttdates=unique(dt.verif$mmestartdate,incomparables = FALSE)
for(k in 1:length(auxmmesttdates)){
  aux=unique(dt.verif[mmestartdate==auxmmesttdates[k],]$startdate,incomparables = FALSE)
  if (length(aux)==2) {
    secondrun=aux[2] #startdate
    dt.verif[mmestartdate==auxmmesttdates[k] & startdate==secondrun]$run=2
  }
}
...
# Ahora sí puedo separar la rlutaem en dos columnas en función de la corrida (Gracias Elio!)
dt.verif=dcast(dt.verif, lat+ lon + targetdate + mmestartdate + L.MME ~ run, value.var = "rlutaem")
setnames(dt.verif,old="1",new="rlutaem.run1")
setnames(dt.verif,old="2",new="rlutaem.run2")
...
#*****
# Tengo que duplicar las filas porque ahora tengo dos corridas!
setnames(obs_append,old="rlutaem",new="rlutaem.run1")
obs_append$rlutaem.run2=obs_append$rlutaem.run1
...
dt.verif = dt.verif[,smt1.olra.run1 := smoothHF(rlutaem.run1),by=.(lon,lat,startdate)]
dt.verif = dt.verif[,smt2.olra.run1 := smoothHF(smt1.olra.run1),by=.(lon,lat,startdate)]

dt.verif = dt.verif[,smt1.olra.run2 := smoothHF(rlutaem.run2),by=.(lon,lat,startdate)]
dt.verif = dt.verif[,smt2.olra.run2 := smoothHF(smt1.olra.run2),by=.(lon,lat,startdate)]

# Bien, ahora descarto lo que no me sirve
dt.verif$rlutaem.run1=NULL
dt.verif$rlutaem.run2=NULL
dt.verif$smt1.olra.run1=NULL
dt.verif$smt1.olra.run2=NULL
setnames(dt.verif,old="smt2.olra.run1",new="EM.run1")
setnames(dt.verif,old="smt2.olra.run2",new="EM.run2")
...
dt.verif = dt.verif[,SIS.index.fcst.run1 := (((sum(EM.run1*stdSISpattern))*SISscale.OA-SISmean.OA)/SISstd.OA),by=.(targetdate,startdate)]
dt.verif = dt.verif[,SIS.index.fcst.run2 := (((sum(EM.run2*stdSISpattern))*SISscale.OA-SISmean.OA)/SISstd.OA),by=.(targetdate,startdate)]


# Armo una dt para el SIS
dt.fcstSIS=dt.verif[lat==-40 & lon==290,.(targetdate,startdate,L,SIS.index.fcst.run1,SIS.index.fcst.run2)]
...
for(j in 1:length(unionsttdates)){
  dt.union[L==0 & startdate==unionsttdates[j]]$SIS.index.fcst.run1=dt.RTSIS[targetdate == unionsttdates[j]]$SIS.index
  dt.union[L==0 & startdate==unionsttdates[j]]$SIS.index.fcst.run2=dt.RTSIS[targetdate == unionsttdates[j]]$SIS.index
}
...
save(dt.fcstSIS, file = "~/SubX_scripts/SubX_SIS_index_forecast_RSMAS.rda")
#--------------------------------------------------------------------
# Gráfico

# Selecciono la temporada a graficar (1999:2013)
for(yi in 1999:2013){
#yi=2009
...
for(f in 1:length(ss)){
  g1 <- g1 + geom_line(data=fcsis[startdate == sttdates[ss[f]]],aes(x=targetdate,y=SIS.index.fcst.run1),color=thecolors[f])
  g1 <- g1 + geom_line(data=dt.union[startdate == sttdates[ss[f]]],aes(x=targetdate,y=SIS.index.fcst.run1),color=thecolors[f],linetype="dashed")

  g1 <- g1 + geom_line(data=fcsis[startdate == sttdates[ss[f]]],aes(x=targetdate,y=SIS.index.fcst.run2),color=thecolors[f])
  g1 <- g1 + geom_line(data=dt.union[startdate == sttdates[ss[f]]],aes(x=targetdate,y=SIS.index.fcst.run2),color=thecolors[f],linetype="dashed")  
...
for(f in 1:length(ss)){
  g2 <- g2 + geom_line(data=fcsis[startdate == sttdates[ss[f]]],aes(x=targetdate,y=SIS.index.fcst.run1),color=thecolors[f])
  g2 <- g2 + geom_line(data=dt.union[startdate == sttdates[ss[f]]],aes(x=targetdate,y=SIS.index.fcst.run1),color=thecolors[f],linetype="dashed")
  
  g2 <- g2 + geom_line(data=fcsis[startdate == sttdates[ss[f]]],aes(x=targetdate,y=SIS.index.fcst.run2),color=thecolors[f])
  g2 <- g2 + geom_line(data=dt.union[startdate == sttdates[ss[f]]],aes(x=targetdate,y=SIS.index.fcst.run2),color=thecolors[f],linetype="dashed")
...
for(f in 1:length(ss)){
  g3 <- g3 + geom_line(data=fcsis[startdate == sttdates[ss[f]]],aes(x=targetdate,y=SIS.index.fcst.run1),color=thecolors[f])
  g3 <- g3 + geom_line(data=dt.union[startdate == sttdates[ss[f]]],aes(x=targetdate,y=SIS.index.fcst.run1),color=thecolors[f],linetype="dashed")
  
  g3 <- g3 + geom_line(data=fcsis[startdate == sttdates[ss[f]]],aes(x=targetdate,y=SIS.index.fcst.run2),color=thecolors[f])
  g3 <- g3 + geom_line(data=dt.union[startdate == sttdates[ss[f]]],aes(x=targetdate,y=SIS.index.fcst.run2),color=thecolors[f],linetype="dashed")
...
for(f in 1:length(ss)){
  g4 <- g4 + geom_line(data=fcsis[startdate == sttdates[ss[f]]],aes(x=targetdate,y=SIS.index.fcst.run1),color=thecolors[f])
  g4 <- g4 + geom_line(data=dt.union[startdate == sttdates[ss[f]]],aes(x=targetdate,y=SIS.index.fcst.run1),color=thecolors[f],linetype="dashed")
  
  g4 <- g4 + geom_line(data=fcsis[startdate == sttdates[ss[f]]],aes(x=targetdate,y=SIS.index.fcst.run2),color=thecolors[f])
  g4 <- g4 + geom_line(data=dt.union[startdate == sttdates[ss[f]]],aes(x=targetdate,y=SIS.index.fcst.run2),color=thecolors[f],linetype="dashed")
}

fig <- grid.arrange(g1,g2,g3,g4, ncol = 1,top = textGrob(paste0("SubX RSMAS EM SIS index forecast season ",as.character(yi),"/",as.character(yi+1)),gp=gpar(fontsize=13,font=3)))
ggsave(filename=paste0("/home/alvarez/SubX_processed_Rdata/SIS_index_forecast_RSMAS_",as.character(yi),"_",as.character(yi+1),".png"),plot=fig,width = 11, height = 14)
