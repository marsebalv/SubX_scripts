# SubX data - olr anomaly verification for NRL model (SubX database)
#
# M. Alvarez - 2020
#-----------------------------------------------------------------------------------------------------------------------
rm(list=ls())
...
dt.verif$L=NULL
# Renombro lf.olra a MME aunque no sea modelo
setnames(dt.olra,old="lf.olra",new="MME")
...
setcolorder(obs_append,neworder=c("lat","lon","targetdate","startdate","MME"))
...
dt.verif = dt.verif[,smt1.olra := smoothHF(MME),by=.(lon,lat,startdate)]
...
setnames(dt.verif,old="smt2.olra",new="MME")
...
dt.verif = dt.verif[,SIS.index.fcst := (((sum(MME*stdSISpattern))*SISscale.OA-SISmean.OA)/SISstd.OA),by=.(targetdate,startdate)]
...
save(dt.fcstSIS, file = "~/SubX_scripts/SubX_SIS_index_forecast_MME.rda")
...
fig <- grid.arrange(g1,g2,g3,g4, ncol = 1,top = textGrob(paste0("SubX MME SIS index forecast season ",as.character(yi),"/",as.character(yi+1)),gp=gpar(fontsize=13,font=3)))
ggsave(filename=paste0("/home/alvarez/SubX_processed_Rdata/SIS_index_forecast_MME_",as.character(yi),"_",as.character(yi+1),".png"),plot=fig,width = 11, height = 14)
