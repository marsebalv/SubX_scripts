  ...
  for(imodel in 1:nmodels){
    ...
    for(iens in 0:(nens-1)){
      ...
      for(i in 0:(nics-1)){
        ...
        ofname=paste0(outDir,varname,'_',as.character(plevstr),'_',group,'-',model,'_',substr(startdates[i+1],1,4),substr(startdates[i+1],6,7),substr(startdates[i+1],9,10),'.e',ee,'.daily.nc')
        ...
        if(ndims == 6){ # multilevel data
          data = metR::ReadNetCDF(inFname, subset = list(M = (iens+1), P = plev, S = as.Date(startdates[i+1])),out='array')
        } else{ # single level data
          data = metR::ReadNetCDF(inFname, subset = list(M = (iens+1), S = as.Date(startdates[i+1])),out='array')
        }
